...
const BOX_TYPE_CFG = {
  ...
  MEDIUM: { letter: 'M', size: 36, fontSize: 12, radius: 7  },
  LARGE:  { letter: 'L', size: 50, fontSize: 15, radius: 10 },
};
...
const KioskLayout = () => {
  ...
  useEffect(() => {
    Promise.all([
      kioskApi.get(`/orders/terminals/${terminalId}/layout`),
      kioskApi.get(`/orders/terminals/${terminalId}/pricing`)
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await kioskApi.post('/orders/book', {
        boxId: selected._id,
  ...
  return (
    <div className="kiosk-viewport" style={{ alignItems: 'flex-start', paddingTop: 40 }}>
      ...
      <div style={{ width: '100%', maxWidth: 720, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap' }}>
          {[['#00E5A0', 'Available'], ['rgba(255,165,0,0.6)', 'Taken']].map(([color, label]) => (
          ...
          {Object.entries(BOX_TYPE_CFG).map(([type, cfg]) => (
            <div key={type} style={{ display: 'flex', alignItems: 'center', gap: 7, color: 'rgba(255,255,255,0.35)', fontSize: 12 }}>
              <div style={{
                width: cfg.size * 0.42, height: cfg.size * 0.42,
          ...
          >
            {rows.map(row => (
              <div key={row} style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
                <span style={{ width: 28, color: 'rgba(123,110,246,0.7)', fontSize: 14, fontWeight: 800, textAlign: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {boxes.filter(b => b.row === row).sort((a, b) => a.col - b.col).map(box => {
                    ...
                    const colors = {
                      available: { bg: 'rgba(0,229,160,0.08)',  border: 'rgba(0,229,160,0.35)',  color: '#00E5A0' },
                      booked:    { bg: 'rgba(255,165,0,0.06)',  border: 'rgba(255,165,0,0.3)',   color: 'rgba(255,165,0,0.55)' },
                      disabled:  { bg: 'rgba(255,255,255,0.02)', border: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.15)' },
                    };
                    const c = isSel
                      ? { bg: 'rgba(123,110,246,0.25)', border: '#7B6EF6', color: 'white' }
                      : colors[status];
                    ...
                    return (
                      <motion.button key={box._id}
                        ...
                        style={{
                          ...
                          color: c.color, cursor: isAvail ? 'pointer' : 'not-allowed',
        ...
        {/* Duration selector — shows after box selected */}
        {selected && pricing && (
          <motion.div
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>
              ...
              <span style={{ marginLeft: 'auto', color: '#7B6EF6', fontSize: 12, fontWeight: 700 }}>
                ₹{pricing[selected.type] || 30}/hr for {selected.type?.charAt(0) + selected.type?.slice(1).toLowerCase()}
              </span>
            </div>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {SLOT_OPTIONS.map(h => (
