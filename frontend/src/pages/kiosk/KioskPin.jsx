...
const KioskPin = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await kioskApi.post(`/orders/pay/${orderId}`);
      await kioskApi.post(`/orders/set-pin/${orderId}`, { pin: pin.join('') });
    ...
    <div className="kiosk-viewport">
      ...
      <motion.div className="kiosk-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="kiosk-input-group">
          ...
          <button onClick={() => setShow(!show)} style={{ background: 'none', border: 'none', color: 'rgba(123,110,246,0.7)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, margin: '8px auto 0', fontFamily: 'Plus Jakarta Sans' }}>
